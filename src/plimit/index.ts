...
import psequence, { type SequenceNext } from '../psequence'
 ...
 * @param {number} [config.limit=1] - The maximum number of promise functions or async functions that can be executed concurrently.
...
export function plimit<T extends PlimitItem[]>(
  ...
  const wrapSlice = (originFns: PlimitItem[]) => {
    return async (next: SequenceNext) => {
  ...
  const runner = psequence(sliceFn(promiseFns))
...
type PlimitReturn<T extends unknown[]> = Promise<{
  [P in keyof T]: T[P] extends Promise<unknown>
    ? Awaited<T[P]>
    : T[P] extends (...args: unknown[]) => Promise<infer P>
    ? Awaited<P>
    : any
}>
type PlimitConfig = {
